...
const APPS = [
  ...
  { id: "slack",     label: "Slack",                icon: "S", color: "from-violet-500 to-purple-400", bg: "bg-violet-500/10", border: "border-violet-500/30", text: "text-violet-300", desc: "Communication équipe" },
...
export default function OnboardingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white flex flex-col items-center justify-center px-6 py-12">
          ...
          <div key={s.id} className="flex items-center gap-3">
            <div className="flex items-center gap-1.5">
              <div className={`rounded-full flex items-center justify-center text-xs font-bold transition-all ${
                i < step ? "w-6 h-6 bg-violet-500 text-white" :
                i === step ? "w-6 h-6 bg-violet-600 text-white ring-2 ring-violet-500/30" :
            ...
            {i < STEPS.length - 1 && <div className={`w-8 h-px ${i < step ? "bg-violet-500/50" : "bg-zinc-800"}`} />}
          </div>
        ))}
      </div>
      ...
      {step === 0 && (
        <div className="w-full max-w-md bg-white/[0.03] border border-white/[0.08] rounded-3xl p-8 text-center">
          ...
          <p className="text-sm text-zinc-400 leading-relaxed mb-6">
            Trigr est ton assistant IA personnel en français. En 2 minutes, il sera prêt à gérer tes emails, agenda et messages à ta place.
          </p>
          ...
          <button onClick={next} className="w-full bg-violet-600 hover:bg-violet-500 text-white font-semibold py-3 rounded-xl transition-all shadow-[0_0_20px_rgba(139,92,246,0.3)]">
      ...
      {step === 2 && (
        <div className="w-full max-w-md bg-white/[0.03] border border-white/[0.08] rounded-3xl p-8 text-center">
          <div className="w-16 h-16 mx-auto mb-6 rounded-2xl bg-violet-600/20 border border-violet-500/20 flex items-center justify-center text-3xl">
            🚀
          </div>
          ...
          <button
            onClick={next}
            className="w-full bg-violet-600 hover:bg-violet-500 text-white font-semibold py-3 rounded-xl transition-all shadow-[0_0_20px_rgba(139,92,246,0.3)]"
          >
